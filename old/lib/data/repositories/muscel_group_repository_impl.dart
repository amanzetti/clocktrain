...
class MuscleGroupRepositoryImpl implements MuscleGroupRepository {
  ...
  @override
  Future<Either<CommonError, List<MuscleGroup>>> getAllMuscleGroups() async {
    final result = await localDbDatasource.getAllMuscleGroups();
    return result.fold(
      (error) => left(CommonError.databaseError),
      (muscleGroupDtos) => right(
        muscleGroupDtos
            .map((muscleGroupDto) => MuscleGroup(
